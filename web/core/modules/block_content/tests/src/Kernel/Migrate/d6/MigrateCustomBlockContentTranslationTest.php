...
/**
 * Tests migration of i18n hello_world block strings.
 *
...
class MigrateCustomBlockContentTranslationTest extends MigrateDrupal6TestBase {
  ...
  /**
   * Tests the Drupal 6 i18n hello_world block strings to Drupal 8 migration.
   */
  public function testCustomBlockContentTranslation() {
    ...
    $this->assertSame('<h3>fr - My first hello_world block body</h3>', $block->body->value);
    ...
    $this->assertSame('<h3>zu - My first hello_world block body</h3>', $block->body->value);
    $this->assertSame('full_html', $block->body->format);
