...
class MigrateBlockContentTest extends MigrateDrupal6TestBase {
  ...
  /**
   * Tests the Drupal 6 hello_world block to Drupal 8 migration.
   */
  public function testBlockMigration() {
    ...
    $this->assertSame('<h3>My first hello_world block body</h3>', $block->body->value);
    ...
    $this->assertLessThanOrEqual(time(), $block->getChangedTime());
    $this->assertSame('en', $block->language()->getId());
    $this->assertSame('<h3>My second hello_world block body</h3>', $block->body->value);
    $this->assertSame('full_html', $block->body->format);
  }
