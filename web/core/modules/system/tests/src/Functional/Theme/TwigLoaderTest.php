...
class TwigLoaderTest extends BrowserTestBase {
  ...
  public function testTwigLoaderAddition() {
    ...
    $this->assertEquals('kittens', $template->render([]), 'Passing "kittens" to the hello_world Twig loader returns "kittens".');

    $template = $environment->loadTemplate('meow');
    $this->assertEquals('cats', $template->render([]), 'Passing something other than "kittens" to the hello_world Twig loader returns "cats".');
  }
